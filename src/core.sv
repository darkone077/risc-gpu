...
module core #(
    ...
    input logic [DATA_LINES-1:0] read_data [0:THREADS-1],
    output logic [ADDR_LINES-1:0] addr[0:THREADS-1],
    output logic [DATA_LINES-1:0] wrt_data [0:THREADS-1],
    output logic [THREADS-1:0] read_wrt,data_req,
    input logic [THREADS-1:0] data_valid,
    ...
    generate
        ...
        for (i=0;i<THREADS;i++) begin: thread_generator
            thread TH(clk,rst_n&thread_en,blk_id,i,read_data[i],addr[i],wrt_data[i],read_wrt[i],data_req[i],data_valid[i],thread_done[i]);
        end
    endgenerate
