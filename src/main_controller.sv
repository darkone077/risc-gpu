module main_controller #(
    ...
    output logic [7:0] block_id [0:BLOCKS-1],
    output logic [7:0] total_threads_per_block [0:BLOCKS-1]
);
    logic [7:0] total_blocks;
    assign total_blocks=(thread_count+THREADS_PER_BLOCK-1)/THREADS_PER_BLOCK;//acts as ceil(thread_count/thread_per_block)
    ...
    always_ff @(posedge clk) begin
        if (~rst_n) begin
            ...
            if ((block_counter==total_blocks)&&(&completed)) begin
                done<=1'b1;
            end
